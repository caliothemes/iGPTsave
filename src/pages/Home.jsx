        ...
        if (template) {
          ...
          if (['logo', 'logo_picto', 'logo_complet'].includes(activeCategory?.id)) {
            ...
            enhancedPrompt += ' --no text --no letters --no words --no typography --no writing';
              ...
              // Design PRINT plein écran - HAUTE QUALITÉ
              enhancedPrompt = `${userMessage}, ultra high quality, extremely detailed, sharp focus, crystal clear, professional photography quality, 8K resolution, maximum detail level, perfect clarity, hyper realistic, intricate details, complete full bleed design filling entire canvas edge to edge, total surface coverage with design elements spreading to all corners and borders, wallpaper style layout covering 100% of area, seamless infinity pattern extending beyond frame, continuous design with no empty margins or white space, professional print-ready full bleed artwork --no border --no white space --no frame --no margin --no padding --no mockup --no card --no centered element --no floating object --no canvas --no mat --no mount --no white background --no empty area --no negative space around design --no perspective --no 3d --no shadow --no text --no letters --no typography --no blur --no low quality --no noise --no grain --no pixelated`;
