...
export default function Home() {
  ...
  useEffect(() => {
    if (currentPromptExamples.length > 0 && !isGenerating && messages.length === 0) {
      setAssistantOpen(true);
    }
  }, [isGenerating, currentPromptExamples, messages.length]);
...
${qaKnowledge}
      ...
      6. CRÉDITS ET TARIFICATION (toujours orienter vers la page Tarifs pour les détails complets):
         ...
         - Génération de vidéo: à partir de 200 crédits (varie selon le modèle, la durée et le format)
           → Pour voir tous les tarifs vidéo et options: clique sur l'icône rouge/rose "Vidéo" sous un visuel généré
      ...
      <main className={cn(
        ...
        {showInitialView ? (
          ...
          <MessagesContainer
            ...
            onEffectApply={async (visual, effect, count) => {
                              ...
                              try {
                                // Déduire count crédits AVANT la génération
                                if (user && credits) {
                                  let creditsToDeduct = count || 1;
                                  let newFreeDownloads = credits.free_downloads;
                                  let newPaidCredits = credits.paid_credits;
                                  ...
                                  const newCount = guestPrompts + (count || 1);
                                  setGuestPrompts(newCount);
                                  localStorage.setItem('igpt_guest_prompts', newCount.toString());
                                }
        ...
        <div className={cn(
          ...
          <div className="relative max-w-2xl mx-auto px-4 pb-6 md:pb-4">
            ...
            <AnimatePresence>
              {currentPromptExamples.length > 0 && !isGenerating && (
                assistantOpen ? (
                  <motion.div
                    initial={{ opacity: 0, y: 20 }}
                    animate={{ opacity: 1, y: 0 }}
  ...
  <div className="mt-3 pt-3 border-t border-violet-500/20">
    ...
    <p className="text-violet-300/60 text-[10px] mt-1.5 hidden md:block">
      {language === 'fr' ? '👆 Cliquez pour ajouter un exemple' : '👆 Click to add an example'}
            ...
            <div className="relative bg-gray-900 border border-white/10 rounded-2xl overflow-hidden prompt-glow">
              ...
              <div className="px-4 pb-2 border-t border-white/5 pt-2">
                ...
                <DropdownMenu>
                  <DropdownMenuTrigger asChild>
                    <button className="px-3 py-1.5 rounded-full text-xs font-medium transition-all border flex items-center gap-1.5 bg-green-600 border-green-500 text-white shadow-lg shadow-green-500/30">
      ...
      <EffectsModal
        ...
        onApplyEffect={async (selectedEffect, variantCount) => {
          ...
          setMessages(prev => [...prev, { 
            role: 'assistant', 
            content: variantCount === 1 
              ? `✨ ${language === 'fr' ? 'Application de l\'effet' : 'Applying effect'} "${effectName}"...`
              : `✨ ${language === 'fr' ? 'Génération de' : 'Generating'} ${variantCount} ${language === 'fr' ? 'variantes de l\'effet' : 'variants of effect'} "${effectName}"...`, 
            isStreaming: true 
          }]);
          ...
          try {
            ...
            const promises = Array(variantCount).fill(null).map(async (_, index) => {
              let finalPrompt = selectedEffect.prompt;
              ...
              if (index > 0) {
                ...
                finalPrompt = `${selectedEffect.prompt}, ${viewVariations[(index - 1) % viewVariations.length]}`;
              }
            ...
            for (let i = 0; i < urls.length; i++) {
              const visualData = {
                ...
                original_prompt: `${visual.original_prompt} • EFFET: ${effectName}`,
                image_prompt: selectedEffect.prompt,
                dimensions: visual.dimensions,
                visual_type: visual.visual_type,
                parent_visual_id: visual.id,
                version: (visual.version || 1) + 1
        ...
        onGenerateVariants={async (selectedEffect, variantCount) => {
          // Appeler la même fonction avec variantCount
          const onApplyFn = arguments.callee.caller?.arguments?.[0]?.onApplyEffect;
          // Utiliser directement onApplyEffect
        }}
      />
